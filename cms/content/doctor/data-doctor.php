...
if($submitcontent=='1'){	
	...
	foreach($_REQUEST as $k=>$v){
		...
		if (in_array(strtoupper($k), $entity['doctor'])){
			$v=str_replace("'","`",$v);
	...
	if($_FILES['IMAGE']['name'] != "") {		
		...
		if (in_array($ext, $allowExt)){
			...
			$target=ROOT_PATH.'/images/doctor/'.$nfname;			
			
			move_uploaded_file($_FILES['IMAGE']['tmp_name'],$target);
			
			//BUAT thumbnail
			if(!create_thumbnail_preserve_ratio($target, ROOT_PATH.'/images/doctor/'.$nfname_thumb, '300')){
				copy($target,ROOT_PATH.'/images/doctor/'.$nfname_thumb);
			}
...
$list=getRecord('tbl_doctor',$params);
        ...
        <div class="card">
            <div class="card-header">
                <a href="<?php echo CMS_URL.'/index.php?page=form-'.$pageseo.'&act=add'?>" type="button"
                    class="btn bg-gradient-primary">Tambah Dokter</a>
