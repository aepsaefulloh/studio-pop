...
if($submitcontent=='1'){	
	...
	foreach($_REQUEST as $k=>$v){
		...
		if (in_array(strtoupper($k), $entity['gallery'])){
			$v=str_replace("'","`",$v);
	...
	if($_FILES['IMAGE']['name'] != "") {		
		...
		if (in_array($ext, $allowExt)){
			...
			$target=ROOT_PATH.'/images/gallery/'.$nfname;			
			
			move_uploaded_file($_FILES['IMAGE']['tmp_name'],$target);
			
			//BUAT thumbnail
			if(!create_thumbnail_preserve_ratio($target, ROOT_PATH.'/images/gallery/'.$nfname_thumb, '300')){
				copy($target,ROOT_PATH.'/images/gallery/'.$nfname_thumb);
			}
...
$list=getRecord('tbl_gallery',$params);
