...
function getRecord($TABLE,$COND){
	...
	foreach($COND as $k=>$v){
		if (($k!='LIMIT') && ($k!='ORDER') && ($k!='KEYWORD') && ($k!='ACT') && ($k!='PAGE') && ($k!='sSearchUser') && ($k!='sSearchContent') && ($k!='NIPOS')&& ($k!='NIC') && ($k!='INC') && ($k!='START_DATE')&& ($k!='END_DATE')&& ($k!='CUSTOM')){
			if($v!=''){
...
function getDoctorURL($objItem){
	// ARTIKEL/ID/TITLE/
	$objItem['TITLE'] = trim(preg_replace('/ +/', ' ', preg_replace('/[^A-Za-z0-9 ]/', ' ', urldecode(html_entity_decode(strip_tags($objItem['NAME']))))));
	
	$search = array("”","`","quot",".","(",")","'", "\"","/", ":", ",", "!", ".", "$", "'", "+", "%", "&",'lsquo;',"rsquo;","?","rlm;",";", " ","<i>","</i>");  
    $replace = array("","","","","","","","-","-","","","","","","","","","","","","","","","-","",""); 
					 
	$seo=str_replace("\\","",(str_replace($search, $replace, $objItem['NAME'])));		
	$result=ROOT_URL.'/dokter/'.$objItem['ID'].'/'.$seo.'/';
	return $result;
}
