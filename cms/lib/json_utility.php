...
if($tbl=='tbl_product'){
	...
	foreach($list['RESULT'] as $list){		
		$posts[] = array('ID'=> $list['ID'],'TITLE'=> $list['TITLE'], 'URL'=> $list['URL'],'TYPE'=> $list['TYPE'],'POS'=> $list['POS'],'ORDNUM'=> $list['ORDNUM'],'LEVEL'=>$list['LEVEL'],'PARENT_ID'=>$list['PARENT_ID']);
	}
	...
	$params['ORDER']='ORDERNUM ASC';
	$list=getRecord($tbl,$params);
	$result['SQL']=$list['SQL'];
	foreach($list['RESULT'] as $list){
		$posts[] = array('ID'=> $list['ID'],'TITLE'=> $list['TITLE'], 'POS'=> $list['POS'], 'URL'=> $list['URL'], 'FILENAME'=> $list['FILENAME'], 'ORDERNUM'=> $list['ORDERNUM']);
	}	
